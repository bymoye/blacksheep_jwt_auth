...
class Gconfig:
    ...
    def fix_config(self) -> None:
        ...
        with open('config.yaml','r+') as f:
            config:dict = yaml.safe_load(f)
            self.password = config['password'] if config and config.get('password') else input('请键入password(加密用): ')
            self.salt = config['salt'] if config and config.get('salt') else os.urandom(16).hex()
            f.seek(0)
            f.write(yaml.safe_dump(asdict(self)))
